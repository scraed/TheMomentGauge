...
from MomentGauge.Statistic.PolyStatistics import Maxwellian_1D_stats, ESBGK_1D_stats
from MomentGauge.Statistic.PolyGaugedStatistics import PolyGaugedStatistics, M35_1D_gauged_stats, Maxwellian_1D_gauged_stats, ESBGK_1D_gauged_stats
...
class ESBGK_Legendre_1D(BaseExpFamilyModel):
    def __init__(self, constant):
        ...
        .. math::
            ...
            \begin{equation}
            \begin{split}
            f_0(\mathbf{u}) &= \exp\left( \beta_0 \phi_0(\mathbf{u}) + \beta_1 \phi_1(\mathbf{u}) + \beta_2 \phi_2(\mathbf{u} )+ \beta_3 \phi_3(\mathbf{u} ) \right)\\
        ...
        self._statistics = ESBGK_1D_stats()
    ...
    def suff_statistics(self, u):
        ...
        float array of shape (4)
            The array :math:`\{ \phi_0(\mathbf{u}),  \phi_1(\mathbf{u}),  \phi_2(\mathbf{u}),  \phi_3(\mathbf{u}) \}`
    ...
    def moments_to_natural_paras(self, betas_ini, moments, domain):
        ...
        Parameters
        ----------
        betas_ini : float array of shape (4)
            A set of proposed natural parameter :math:`\boldsymbol{\beta}`
        moments : float array of shape (4)
    ...
    @partial(jax.jit,
             static_argnums=0)
    def rhoVTsigma_to_natural_paras(self, rhoVTsigma, domain, Pr = 2/3):
        ...
        Parameters
        ----------
        rhoVTsigma : float array of shape (4)
            A float array ( :math:`\rho`, :math:`v_x`, :math:`T`, :math:`\sigma_xx` ) 
        ...
        beta0=1/2*(-((m*n*Pr*vx**2)/(kB*n*Pr*T+(-1+Pr)*sigma_xx))+5*Log(n)+3*Log((m*Pr)/pi)-Log(2*(kB*n*Pr*T+(-1+Pr)*sigma_xx))-Log( (2*kB*n*Pr*T+sigma_xx-Pr*sigma_xx)**2 ))
        beta1 = (m*n*Pr*vx)/(kB*n*Pr*T+(-1+Pr)*sigma_xx)
        beta2 = -((m*n*Pr)/(2*(kB*n*Pr*T+(-1+Pr)*sigma_xx)))
        beta3 = -(m*n*Pr)/(2*kB*n*Pr*T+sigma_xx-Pr*sigma_xx)
        
        betas = jnp.asarray([beta0, beta1, beta2, beta3])
...
class ESBGK_Canonical_Legendre_1D(ESBGK_Legendre_1D,CanonicalExpFamilyModel):
    def __init__(self, constant):
        super().__init__(constant)
    ...
    def rhoVTsigma_to_natural_paras(self, rhoVTsigma, domain, Pr = 2/3):
        ...
        gauge_paras = ()
        base_args = (domain,)  
        betas = super().rhoVTsigma_to_natural_paras(rhoVTsigma, domain, Pr = Pr)
        betas = self._ori_to_canon_parameter_convertion(betas, gauge_paras = gauge_paras, base_args = base_args )
...
class ESBGK_Gauged_Legendre_1D(GaugedExpFamilyModel, BaseExpFamilyModel):
    def __init__(self, constant):
        ...
        .. math::
            ...
            \begin{equation}
            \begin{split}
            f_0(\mathbf{u}) &= \exp\left( \beta_0 \phi_0(\mathbf{u},s_r, s_x, w_x) + \beta_1 \phi_1(\mathbf{u}, s_r, s_x, w_x) + \beta_2 \phi_2(\mathbf{u} , s_r, s_x,, w_x) + \beta_3 \phi_3(\mathbf{u} , s_r, s_x,, w_x) \right)\\
            \end{split} 
            \end{equation}
        ...
        in which :math:`\rho` is the number density, :math:`m` is the molecule velocity, :math:`v_x` is the flow velocity at the x direction, :math:`T` is the temperature, and
            ...
            :math:`\phi_0` (**u**, :math:`s_r, s_x, w_x` ) = 1.

            :math:`\phi_1` (**u**, :math:`s_r, s_x, w_x` ) = :math:`\bar{u}_x`
            
            :math:`\phi_2` (**u**, :math:`s_r, s_x, w_x` ) = :math:`\bar{u}_x^2`

            :math:`\phi_3` (**u**, :math:`s_r, s_x, w_x` ) = :math:`\bar{u}_y^2 + \bar{u}_z^2`.
        ...
        super().__init__(constant)
        self._Qsampler = Gauss_Legendre_Sampler_Polar3D(n_x = constant['n_x'],n_r = constant['n_r'], B_x = constant['B_x'], B_r = constant['B_r'])
        self._statistics = ESBGK_1D_gauged_stats()
        self._estimator = EstimatorPolar2D(constant)
        self._sampler = ExpFamilyImportanceSampler(self._statistics.suff_stats, self._Qsampler )
        self._optimizer = Newton_Optimizer(self._Loss,alpha = constant["alpha"], beta = constant["beta"], c = constant["c"], atol = constant["atol"], rtol = constant["rtol"],max_iter=constant["max_iter"], max_back_tracking_iter = constant["max_back_tracking_iter"], tol = constant["tol"], min_step_size = constant["min_step_size"], reg_hessian = constant["reg_hessian"], debug = constant["debug"] )
    @partial(jax.jit,
    ...
    def suff_statistics(self, u, gauge_paras):
        ...
        gauge_paras : float array of shape (3)  
            The array (:math:`s_r, s_x`, :math:`w_x`).

        Returns
        -------
        float array of shape (4)
            The array :math:`\{ \phi_0(\mathbf{u}, s_r, s_x, w_x),  \phi_1(\mathbf{u}, s_r, s_x, w_x),  \phi_2(\mathbf{u}, s_r, s_x, w_x), \phi_3(\mathbf{u}, s_r, s_x, w_x) \}`
    ...
    def moments_to_natural_paras(self, betas_ini, moments, gauge_paras, domain):
        ...
        Tuple
            A tuple containing
                ...
                **beta**: *float array of shape (4)* - The natural parameters :math:`\boldsymbol{\beta}`
    ...
    def moments_gauge_transformation(self,moments, gauge_para2, gauge_para1, domain):
        ...
        .. math::
            ...
            \begin{equation}
                M_i( s_r', s_x', w_x' )= T_{ij}(s_r', s_x', w_x', s_r, s_x, w_x) M_j(s_r, s_x, w_x); \quad i,j = 0, \cdots, 3
            \end{equation}       
        ...
        .. math::
            ...
            \begin{equation}
                \phi_i(\mathbf{u}, (s_r', s_x',w_x') )= T_{ij}(s_r', s_x', w_x',s_r, s_x, w_x) \phi_j(\mathbf{u}, (s_r, s_x, w_x)); \quad i,j = 0, \cdots, 3
            \end{equation}
    ...
    def natural_paras_gauge_transformation(self,betas, gauge_para2, gauge_para1, domain):
        ...
        .. math::
            ...
            \begin{equation}
                \beta_i( s_r', s_x', w_x' )= T_{ij}^T(s_r, s_x, w_x, s_r', s_x', w_x') \beta_j(s_r, s_x, w_x); \quad i,j = 0, \cdots, 2
            \end{equation}        
        ...
        .. math::
            ...
            \begin{equation}
                \phi_i(\mathbf{u}, (s_r', s_x', w_x') )= T_{ij}(s_r', s_x', w_x',s, w_x) \phi_j(\mathbf{u}, (s_r, s_x, w_x)); \quad i,j = 0, \cdots, 2
            \end{equation}
    ...
    @partial(jax.jit,
             static_argnums=0)
    def rhoVTsigma_to_natural_paras(self, rhoVTsigma, gauge_paras, domain, Pr = 2/3):
        r"""Compute the natural parameters :math:`\boldsymbol{\beta}` from the density :math:`\rho`, flow velocity :math:`v_x` and temperature :math:`T`
...
class ESBGK_Canonical_Gauged_Legendre_1D(ESBGK_Gauged_Legendre_1D,CanonicalExpFamilyModel):
    def __init__(self, constant):
        super().__init__(constant)
        self._sampler = CanonicalExpImportanceSampler(self._statistics.suff_stats, self._Qsampler )
    @partial(jax.jit,
...
if __name__ == "__main__":
    ...
    rhoVT = jnp.array([1,0,3,0.3])
    gauge = jnp.array([1,1,0.]) 
    Maxwell = ESBGK_Legendre_1D(constant)
    Maxwell2 = ESBGK_Canonical_Legendre_1D(constant)
    Maxwell3 = ESBGK_Gauged_Legendre_1D(constant)
    Maxwell4 = ESBGK_Canonical_Gauged_Legendre_1D(constant)

    beta1 = Maxwell.rhoVTsigma_to_natural_paras(rhoVT,domain_para)
    print(beta1)
    beta2 = Maxwell2.rhoVTsigma_to_natural_paras(rhoVT,domain_para)
    print(beta2)
    beta3 = Maxwell3.rhoVTsigma_to_natural_paras(rhoVT,gauge,domain_para)
    print(beta3)
    beta4 = Maxwell4.rhoVTsigma_to_natural_paras(rhoVT,gauge,domain_para)
    print(beta4)

    flowprops = Maxwell.natural_paras_to_fluid_properties(beta1, domain_para)
    print(flowprops[7])
    '''
