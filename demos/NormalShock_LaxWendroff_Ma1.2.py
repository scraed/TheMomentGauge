...
PhyConst = PhyConstants(Ma=1.2,velocity_std_ratio=(6,6,6))

constant = {"m": PhyConst.m, "kB": PhyConst.kB,"n_x": 8, "n_r": 8, "B_x": 2, "B_r": 2 ,
...
CFL_ratio = 0.5 # If nan appears, make it smaller
...
def gauged_Limiter( values_left, values_middle, values_right):
    ...
    phi_vec = vmap( MinmodLimiter )(rvec)
...
@jax.jit
def gauged_Richtmyer_States( values_left, values_right, delta_x, delta_t):
    """
    Calculate the Lax_Friedrichs_Flux at a cell interface with gauge transformation considered.
    """
    paras_left, moment_left, flux_left, gauges_left, domain_left, speed_left = values_left
    ...
    paras_middle = (paras_left+paras_right)/2
    ...
    return flux_middle_left, flux_middle_right, opt_info
...
def transport(moments, paras, gauges, domain, delta_t, verbose = False):
    ...
    flux_middle_left, flux_middle_right, opt_info = vmap( gauged_Richtmyer_States, in_axes =  ( (0,0,0,0,None,0),(0,0,0,0,None,0), None, None ) )(values_left, values_right, delta_x, delta_t)
    ...
    right_right_index = np.concatenate( ( np.arange(2, len(paras)), [len(paras)-1] ) , axis = 0)
    ...
    if verbose:
        jax.debug.print("Phivec plus: {x}", x=phi_vec_plus_half )
    ...
    return Transport_terms, paras
@partial(jax.jit, static_argnames=['verbose'])
def collision(moments, paras, gauges, domain, delta_t, verbose = False):
    moments_O, paras_O, gauges_O, domain_O = moments, paras, gauges, domain
    moments, paras, gauges, domain = ToHermiteGauge(moments, paras, gauges, domain)
    paras, opt_info = vmap( M35.moments_to_natural_paras, in_axes = (0,0,0,None) )( paras, moments, gauges ,domain)
    moments, paras, gauges, domain = GaugeTransformation(moments, paras, gauges_O, gauges, domain)    
    ...
    return moment_collisions_term[1:-1], paras
@partial(jax.jit, static_argnames=['verbose'])
def step( moments, paras, gauges, domain, delta_t, verbose = False ):

    def RK2( func, moments, paras, gauges, domain, delta_t, verbose = False ):
        dmoment, paras = func(moments, paras, gauges, domain, delta_t, verbose = verbose)
        moments1 = moments.at[1:-1].set(moments[1:-1]  + delta_t*dmoment )
        dmoment1, paras1 = func(moments1, paras, gauges, domain, delta_t, verbose = verbose)
        moments = moments.at[1:-1].set(moments[1:-1]  + delta_t/2*(dmoment + dmoment1) )
        return moments, paras1, gauges, domain

    moments, paras, gauges, domain = ToHermiteGauge(moments, paras, gauges, domain)
    moments, paras, gauges, domain = RK2(collision, moments, paras, gauges, domain, delta_t/2, verbose = verbose)
    moments, paras, gauges, domain = RK2(transport, moments, paras, gauges, domain, delta_t, verbose = verbose)
    moments, paras, gauges, domain = RK2(collision, moments, paras, gauges, domain, delta_t/2, verbose = verbose)
    ...
    return (moments, paras, gauges, domain), delta_t2, opt_info
...
if __name__ == "__main__":
    ...
    print("Jit Compile")
    print("*********************************************************")
    step(*M35_distribution_infos, delta_t, verbose=True)
    step(*M35_distribution_infos, delta_t, verbose=False)
    print("*********************************************************")
    ...
    for i in range(1000000000+1):
        time += delta_t
        if i%record_steps == 0:
            M35_distribution_infos, delta_t, opt_info = step(*M35_distribution_infos, delta_t, verbose=True)
        else:
            M35_distribution_infos, delta_t, opt_info = step(*M35_distribution_infos, delta_t, verbose=False)
        if time > 25:
            break
        if i%record_steps == 0:
